...
public class AgendaDataSource {
	...
	public Event getEvent(long id) {
        ...
        cursor = database.query(SQLiteHelper.TABLE_EVENTS, null, "_id = ?", new String[] { Long.toString(id)}, null, null, null);
	...
	private Event cursorToEvent(Cursor cursor) {
		...
		int id_index = cursor.getColumnIndex(SQLiteHelper.COLUMN_ID);
        int title_index = cursor.getColumnIndex(SQLiteHelper.COLUMN_TITLE);
        int location_index = cursor.getColumnIndex(SQLiteHelper.COLUMN_LOCATION);
        int start_index = cursor.getColumnIndex(SQLiteHelper.COLUMN_START);
        int end_index = cursor.getColumnIndex(SQLiteHelper.COLUMN_END);
        int details_index = cursor.getColumnIndex(SQLiteHelper.COLUMN_DETAILS);

        event.setId(cursor.getLong(id_index));
        event.setTitle(cursor.getString(title_index));
        event.setStartTime(cursor.getString(start_index));
        event.setEndTime(cursor.getString(end_index));
        event.setDetails(cursor.getString(details_index));
